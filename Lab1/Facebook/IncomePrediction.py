...
def TrainModels(yearFrom, yearTo, variableFrom, variableTo):
    ...
    xForExponential = sm.add_constant(trainX.reshape(-1, 1))
    ...
    exponentialModel_statsmodels = sm.OLS(yForExponential, xForExponential).fit()
    ...
    xForPower = sm.add_constant(np.log(trainX).reshape(-1, 1))
    ...
    powerModel_statsmodels = sm.OLS(yForPower, xForPower).fit()
    ...
    xForPolynomialModel = sm.add_constant(np.concatenate((x1, x2, x3, x4), axis=1))
    ...
    polynomialModel_statsmodels = sm.OLS(yForPolynomialModel, xForPolynomialModel).fit()
    ...
    x1 = trainX.reshape(-1, 1)
    x2 = np.power(trainX, 2).reshape(-1, 1)
    xForSquareModel = sm.add_constant(np.concatenate((x1, x2), axis=1))
    ...
    squareModel_statsmodels = sm.OLS(yForSquareModel, xForSquareModel).fit()
    ...
    xForLinear = sm.add_constant(trainX.reshape(-1, 1))
    ...
    linearModel_statsmodels = sm.OLS(yForLinear, xForLinear).fit()
...
def TestModels(yearFrom, yearTo):
    ...
    testX = sm.add_constant(testData[independentVariableName].values.reshape(-1, 1))
    meanSquareErrorsForModels = []

    testData['Wykładniczy ręczny'] = np.exp(testData[independentVariableName].values * fit[0] + fit[1])
    ...
    testData['Wykładniczy'] = np.exp(exponentialModel.predict(testX))
    ...
    testData['Potęgowy'] = np.exp(powerModel.predict(np.log(testX)))
    ...
    xt1 = testData[independentVariableName].values.reshape(-1, 1)
    xt2 = np.power(xt1, 2).reshape(-1, 1)
    xt3 = np.power(xt1, 3).reshape(-1, 1)
    xt4 = np.power(xt1, 4).reshape(-1, 1)
    X2 = sm.add_constant(np.concatenate((xt1, xt2, xt3, xt4), axis=1))
    ...
    xt1 = testData[independentVariableName].values.reshape(-1, 1)
    xt2 = np.power(xt1, 2).reshape(-1, 1)
    X2 = sm.add_constant(np.concatenate((xt1, xt2), axis=1))
    ...
    testData['Liniowy'] = linearModel.predict(testX)
